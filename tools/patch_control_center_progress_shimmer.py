...
old_html = "'<div class=\"bigprogress\">'+progress(p.progress)+'<div class=\"percent\">'+(p.progress||0)+'%</div></div>"
new_html = "'<div class=\"bigprogress '+(!terminal.includes(p.status)?'live':'')+'\">'+progress(p.progress)+'<div class=\"percent\">'+(p.progress||0)+'%</div></div>"
if old_html in text:
